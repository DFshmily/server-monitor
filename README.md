...
### 🔄 系统更新提醒
- agent 每小时统计 `apt` 可升级包数量并采集具体包列表，首页卡片显示"待更新 N 个"，
  登录用户可点击展开查看具体哪些软件待更新(包名+新旧版本)，超过200个自动截断
...
### 🖥 双机对比 + 时段对比
- **双机对比**：同一指标两台服务器实时走势叠加
- **时段对比**（同环比）：今 vs 昨 / 本周 vs 上周曲线平移对齐 + 均值变化百分比，"今天为什么变卡"一眼看出异常时段
...
- **TCP 端口** — 端口是否开放可连接
- **SSL 证书** — 证书有效期探测（默认 443），剩 <15 天自动转红；配合告警规则"证书剩余天数"提前换证
- **Ping / DNS** — 可达性与解析
...
### 🛠 告警系统
- 阈值规则（CPU / 内存 / 磁盘 / 负载 / 网络 / 证书天数 / 月流量 / 磁盘温度 / 进程守护）+ 恢复通知 + 30 分钟冷却
- **进程守护**：规则指标选 `process:进程名`（如 nginx），进程数 < 阈值即告警，关键服务"悄悄挂了"立刻知道
- **告警聚合**：5 分钟窗口内同源告警合并为一条推送（附触发次数），网络抖动不再轰炸
- **持续告警升级**：条件持续 30 分钟未恢复 → ⚠️ 升级提醒（之后每小时重复），避免"看了就忘"
- 离线检测（心跳超时）+ 恢复通知
- **维护模式**：窗口内对应服务器告警静默（事件仍记录）
- **通知渠道**：Telegram（MarkdownV2 富文本卡片 + 直达详情页链接）/ Bark；Telegram 推送失败自动降级纯文本，告警永不丢失
- 告警事件记录 + 管理操作审计
...
### 💽 磁盘硬件健康（S.M.A.R.T.）
- 详情页「硬件健康」tab：物理盘温度 / 重映射扇区 / 待映射扇区 / 通电时长 / 整体健康状态
- 告警规则支持"磁盘温度"指标（如 > 60°C 提醒）
- 云主机虚拟盘或未装 smartctl 时自动留空，不报错

### 📱 PWA（添加到主屏幕）
- 面板支持 PWA：手机浏览器"添加到主屏幕"后像原生 App 一样全屏秒开
- Service Worker 缓存静态资源，弱网/离线时仍能打开外壳（实时数据始终走网络不缓存）
- 支持 iOS 横幅与桌面图标（manifest + apple-touch-icon）
